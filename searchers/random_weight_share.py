...
class Node:
    ...
    def to_dict(self):
        out = {'parent': self.parent, 'arch': self.arch,
               'node_id': self.node_id, 'rung': self.rung}
...
class Random_NAS:
    ...
    def print_summary(self):
        ...
        objective_vals = [(n, self.arms[n].objective_val)
                          for n in self.arms if hasattr(self.arms[n], 'objective_val')]
...
def main(args):
    ...
    if args.save_dir is None:
        save_dir = os.path.join(root_dir, '%s/random/trial%d' %
    ...
    if args.benchmark == 'ptb':
        ...
        model = DartsWrapper(save_dir, args.seed, args.batch_size,
                             args.grad_clip, config=args.config)
    elif args.benchmark == 'cnn':
        from benchmarks.cnn.darts.darts_wrapper_discrete import DartsWrapper
        model = DartsWrapper(save_dir, args.seed, args.batch_size,
                             args.grad_clip, args.epochs, init_channels=args.init_channels)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Args for SHA with weight sharing')
    ...
    parser.add_argument('--init_channels',
                        dest='init_channels', type=int, default=16)
